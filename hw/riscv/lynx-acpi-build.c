                       ...
                       RISCVVirtState *s)
{
    ...
    if (socket_count == 1) {
        virtio_acpi_dsdt_add(scope, memmap[LYNX_VIRTIO].base,
                             ...
                             LYNX_VIRTIO_IRQ, 0, VIRTIO_COUNT);
                       ...
                       RISCVVirtState *s)
{
    ...
    MachineState *ms = MACHINE(s);
    const CPUArchIdList *arch_ids = mc->possible_cpu_arch_ids(ms);
    uint8_t  group_index_bits = lynx_imsic_num_bits(riscv_socket_count(ms));
    uint8_t  guest_index_bits = lynx_imsic_num_bits(s->aia_guests + 1);
    ...
    hart_index_bits = lynx_imsic_num_bits(imsic_max_hart_per_socket);
...
static void lynx_acpi_build(RISCVVirtState *s, AcpiBuildTables *tables)
{
    ...
    if (lynx_is_iommu_sys_enabled(s) || s->pci_iommu_bdf) {
        acpi_add_table(table_offsets, tables_blob);
